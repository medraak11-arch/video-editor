...
src/state/mediaSlice.ts            ONE changed argument (§7.4). Without it §11.27 fails.
scripts/check-export-graph.mjs     NEW file (§10.6). The gate §11.24 declares non-optional.
package.json                       ONE string: the new script appended to `check` (§10.6).
```

Everything above the comment is this area's outright. The four below it are **single-edit exceptions,
and they are part of this change rather than requests to another owner** — the brief for this run
states nothing else is running, so a nominated owner would never apply them and the feature would be
unlandable rather than merely degraded. Each is spelled out as literal text at the section named
beside it; three are pure additions and the fourth changes one argument and the comment above it.
...
ladder — `2160, 1440, 1080, 720, 480` — generates every resolution in the app, for every shape.
...
| Preview and export agree on the composited result | **Almost** | Three real gaps: §5.4 (matte colour), §6.2 (position offset drifts when export resolution ≠ project resolution), §4.3 (rotation, already an EXPORT §7 omission) |

So the substantial half of this work is **not** implementing letterboxing. It is: giving the user a
way to *choose* the shape, making the frame boundary visible now that bars are the normal case, and
closing the three places where the two engines disagree.
...
A shape with no preset (§3.2) still gets a ladder — the same five tiers at that exact aspect — but
no tier names, so its options render as the pixel pair alone. Nothing is unreachable for want of a
name.
...
2. **It is the only candidate that would break the rule this document exists to enforce.** 17:9 is
   ...
   §6.3 closes.
3. **Cutting it removes nothing.** A user who genuinely needs 4096 × 2160 types it into the Width
   and Height fields in the inspector. The project becomes a 17:9 project, the preview letterboxes
   to 17:9, and the export ladder is generated from 17:9 with 4096 × 2160 at its 2160 tier. The
   capability survives; only the misleading menu entry is gone.
...
what makes point 3 true rather than a consolation. A 17:9 project's own ladder does contain
`2048 × 1080` at its 1080 tier, unnamed, because that is genuinely 17:9's 1080 tier. §11.6 is
 ...
 * in, so nothing in this list can change the shape of the output (§6.3).
 *
 ...
 * reachable from that project at all. One ordinary selection would destroy the named
 * ladder, and §6.3 feeds this same list to the export dialog. Resolving the preset first
 * makes a preset project regenerate its own canonical ladder from any of its tiers, so
 * the ladder is idempotent (§11.4). A 'custom' shape has no canonical ratio and keeps the
 * live one.
 *
...
| `resolveAspectId(4096, 2160)` | `'custom'` (1.8889, 0.111 outside 16:9) |
...
| `resolutionLadder(1920, 1081)[0]` | `1920 × 1082`, unnamed, above the five 16:9 tiers. The store still holds 1081. |
...
| `resolutionLadder(4096, 2160)` | five unnamed rows, `4096 × 2160` at tier 2160 — §2.3's escape hatch, intact |
...
who picks `Landscape 16:9` gets `1778 × 1000` — landscape, short edge 1000 — and the Resolution select then shows that size as its
own first row above the named ladder. Snapping would silently change a number the user typed on
purpose, and this area does not do that to anyone.
...
scoped deliberately, and §7.3 shows where it stops: an `adoptSourceFormat` that takes a *rate* can
shorten a clip on a track below the fold, which is exactly silent, so that path does owe a notice.
...
They are the same operation expressed twice, and §6 verifies they produce the same pixels.
...
### 5.4 The matte must be the colour the export writes

The export's base canvas is `color=c=black` (`EXPORT.md §1.2`), so bars in the delivered file are
black. The preview's bars are `--surface-well`, which is `oklch(0.10 0.008 265)` in `signal`,
`oklch(0.10 0 0)` in `instrument` and `oklch(0.13 0 0)` in `daylight`. **Three different bar colours,
none of them the shipped one.**
...
§11.30, i.e. the whole change is unlandable. So the token is declared in this change and the CSS
...
§11.16). The second run therefore takes the guard and returns. That guarantee is the reason the
...
5. **§6 Gates**, add: the §1.8 transcripts are no longer diffed by eye. `scripts/check-export-graph.mjs`
   rebuilds all three and diffs `filterScript` and `args` against the literals in §1.8, plus a fourth
   case at double the document resolution that pins the placement rescale above. It runs inside
   `npm run check` (FORMAT §10.6), so a change to `offset()`'s output or to the ratio arithmetic fails
   a gate rather than a reading.
...
**Why the notice and the re-seek are owed here, when the shipped `adoptSourceFormat` owes neither.**
Today `setProjectSize` sets `formatLocked`, so choosing a shape *structurally* prevents any later
rate adoption and `adoptSourceFormat` can only ever run on a project nobody has touched. The split
removes that: choosing a shape now sets only `sizeLocked`, so a rate adoption can land on a project
that already has clips laid out. Concretely — set Aspect to `Vertical 9:16`, import an audio file
(`mediaSlice`'s adopt guard requires `kind === 'video'`, so nothing locks), lay out several minutes
of audio at the default 30 fps, then import 24 fps video. The rate adopts, every media
`durationFrames` shrinks by 20%, and `clampClipsToSource` — the one function in this codebase
licensed to shorten a clip the user did not ask to shorten — truncates those audio clips.

§3.6's "nothing is silent here, the preview well changes shape in the same frame" covers a *size*
change and does not cover this: a truncated clip on a track below the fold is exactly silent. So
`adoptSourceFormat` now reports the trim in the same words `setProjectFps` uses, and re-seeks for the
same reason `setProjectFps` re-seeks — the tail clamp can move under the playhead.
...
and a caller with dimensions but no rate adopts the shape only. §7.4 carries the one call site this
contract requires changing.
...
| Sequence | Result |
|---|---|
| Set `9:16` → import 1920×1080 @ 24 | `1080 × 1920 @ 24`. Shape kept, rate adopted. |
| Import 1920×1080 @ 24 → set `9:16` → import another | `1080 × 1920 @ 24`. Explicit shape survives the second import. |
| Set fps 25 → import 1920×1080 @ 24 | `1920 × 1080 @ 25`. Rate kept, shape adopted. |
| Open a `.veproj` → import anything | Nothing adopts. Both halves are explicit. |

**An explicit choice always wins and is never silently overwritten.**

Every row above assumes a source whose rate was actually **measured** — i.e. Electron's `ffprobe`
path. On the `dev:web` browser-drop path no rate exists to adopt (§7.4), so row 1 reads
`1080 × 1920 @ 30` there, with `fpsLocked` still `false` and the rate still open. That is the honest
answer, not a shortfall: nothing measured the source, so nothing was adopted. §11.27 asserts both.

### 7.4 `src/state/mediaSlice.ts` — exactly one changed argument

**The guards do not change.** `mediaSlice` guards its two `adoptSourceFormat` calls with
`!get().formatLocked`. Because `formatLocked` is maintained as `fpsLocked && sizeLocked`, that guard
now reads "at least one half is still open", which is exactly when the call is worth making.
`adoptSourceFormat` then decides, per half, what to take. This is why `formatLocked` is retained
rather than renamed: renaming it would mean patching call sites across a slice, which is precisely
the cross-area collision that has already cost this project a build once.

**One argument does change, and it is not optional.** `importBrowserFile` — the browser-drop path,
the one `dev:web` uses — calls:
...
defect: `takeFps = !s.fpsLocked && m.fps > 0` is unconditionally true there, so the browser path
sets `fpsLocked: true` from a number nobody measured, permanently blocking the project from ever
adopting a real source rate, and it runs `recomputeMediaDurations` + `clampClipsToSource` for a rate
that did not move. It also falsifies §11.27 on the very harness §11 nominates: `setProjectSize(1080,
1920)` then importing 24 fps footage through the browser path would leave `fps: 30, fpsLocked: true`.
...
The Electron path (`applyProbe`) passes a real probed `data.fps` and is **unchanged**.

`src/state/mediaSlice.ts` is on the §0 ownership list for this one argument. Nothing else in the file
...
2. **§3.3** declares `adoptSourceFormat` as "One-shot auto-adopt from the first ready item. No-op
   ...
   half is still unlocked; a source rate of `0` means "unknown" and adopts nothing; and — new, and
   the reason this is an amendment rather than a clarification — **adopting a rate can now raise a
   `Notice`**, because with the locks split a rate adoption can land on a project that already has
   clips laid out, and `clampClipsToSource` may shorten them (§7.3).
...
- **Contrast.** No new text colour. Two new painted things, both checked. `.ve-video-stage`'s
  ...
  themes, which is item §11.18's measurement.
...
- **The Icon Tax Rule.** No new status *surface* is introduced. The two places status appears are
  both existing primitives that already carry icon + text and derive colour third: `mediaSlice`'s
  `resolution-mismatch` warning, and the `warning` `Notice` §7.3 adds to `adoptSourceFormat` — which
  reuses `setProjectFps`'s exact title and message strings, so no new copy and no new tone is
  introduced either. Nothing in this feature encodes status in colour alone.
...
- **Applied in this change**, because the feature does not land without them and nothing else is
  ...
  §7.4's one changed argument in `mediaSlice.ts`. All three are on the §0 ownership list, each for a
  single edit.
...
1 exempts, and §11.31's grep excludes it.
...
Not owned. Two entries for whoever owns it:

- Under features: project shape is chosen in the inspector, five presets plus custom, and export
  resolutions are named and locked to the project aspect.
- Under **Known limitations**: changing the project format is not undoable (§3.6), and a mismatched
...
**Applied here.** §11.24 calls the byte-for-byte diff of `EXPORT.md §1.8`'s three transcripts "the
...
New file, mirroring the shape of the three scripts already in `scripts/`: no dependencies, plain
`node`, non-zero exit on failure. It imports `dist-electron/electron/export/graph.js` — the build
output, so `npm run build` precedes it exactly as it does for the Electron gates — rebuilds each
request, and diffs `filterScript` and `args` against the literals held in `EXPORT.md §1.8`.

**Four cases, not three.** A, B and C are the §1.8 transcripts, all `req` = `doc` = 1920 × 1080,
where `rx === ry === 1` and §6.2 is a provable no-op. A fourth case makes §11.23 a gate rather than a
...
Every item is a measurement, not an inspection. `dev:web` items run against the 41-clip fixture
project; Electron items are built through CDP per the project's standard harness. Assert
`document.visibilityState === 'visible'` in every sample that touches playback.
...
3. `resolutionLadder(w, h).every(o => Math.abs(o.width / o.height - w / h) < 0.01)` is `true` for
   every ladder in §2.2 and for `4096 × 2160`.
4. **The ladder is idempotent — the check items 1 and 2 cannot make, because they only ever run
   against a pristine store.** For each of the five options of `resolutionLadder(1920, 1080)`: call
   `setProjectSize(o.width, o.height)`, then assert
   `resolutionLadder(readStore().width, readStore().height).map(o => o.label)` equals item 1's five
   labels in the same order. Repeat from `1080 × 1920` against item 2, and from `1080 × 1350`
   against §2.2's 4:5 table.
...
5. Every `width` and `height` in every ladder is even, **including the passthrough row**:
   `resolutionLadder(1920, 1081)[0]` is `{ width: 1920, height: 1082 }` and its label is exactly
   `'1920 × 1082'` — no tier name — while `readStore().height` is still `1081`.
6. Generator inputs, not rendered labels: `JSON.stringify(ASPECT_PRESETS)` and
   ...
   whose label is exactly `'4096 × 2160'` — present, and unnamed. Item 6 is about the generator
   ...
   promises survives.
7. No ladder entry exceeds `SIZE_MAX` on either axis. `resolutionLadder(16, 8192)` has length 1 and
   is the project's own size — every tier wants over 16384 px on one axis and is skipped, and the
   project's size is still reachable.
8. `resolutionLabel(1920, 1084) === '1920 × 1084'` and `resolutionLabel(1922, 1080) === '1922 × 1080'`
   — both inside `ASPECT_EPSILON` of 16:9, neither borrowing a name it has not earned.
9. The export dialog's Resolution `<select>` carries `data-numeric` and its computed
   `font-family` resolves to `var(--font-mono)`.

**Project format**

10. With nothing selected, the inspector's Project group renders five rows in the order Aspect,
    Resolution, Width, Height, Frame rate.
11. Setting Aspect to `Vertical 9:16` on a `1920 × 1080` project produces `width === 1080` and
    `height === 1920` in one store write.
12. `Custom` is absent from the Aspect options while `resolveAspectId(w, h) !== 'custom'`, and
    present-and-selected when it is.
13. Typing `1081` into Height commits `1082`. `readStore().height % 2 === 0` after any
    `setProjectSize`.
14. A format change leaves `JSON.stringify(readStore().clips)` byte-identical, and leaves
    `readStore().history.past.length` unchanged.
15. `readStore().isDirty === true` after a format change that moved a dimension — **and unchanged
    after one that did not.** From a store with `isDirty === false` and `sizeLocked === false` (save
    first if the fixture project opens dirty), select `Landscape 16:9` on the 1920 × 1080 default:
    `readStore().isDirty` is still `false` and `readStore().sizeLocked` is now `true`. Confirming the
    shape you already have must not arm the quit prompt.
16. Every Resolution `<select>` holds a value that exists in its own options. For `(w, h)` in
    `1920 × 1080`, `1080 × 1920`, `1920 × 1081` and `4096 × 2160`:
    `resolutionLadder(w, h).some(o => o.value === projectResolutionValue(w, h))` is `true`.

**The preview**

17. In a `1080 × 1920` project, `.ve-video-stage`'s `getBoundingClientRect()` satisfies
    `Math.abs(w / h - 1080 / 1920) < 0.01`. Repeat for `1:1` and `4:5`.
18. `getComputedStyle(stage).outlineWidth === '1px'` and its `outlineColor` resolves to
    `--border-structural`, in all three themes.
19. `getComputedStyle(document.documentElement).getPropertyValue('--frame-matte').trim() !== ''`, and
...
20. With a landscape clip under the playhead in a vertical project, the `<video>` element's
...
21. Changing the project format while a clip is under the playhead produces no console error and no
...
22. `buildExportGraph` for a `1920 × 1080` source in a `1080 × 1920` project at `1080 × 1920` emits
    `scale=1080:1920:force_original_aspect_ratio=decrease…` and an overlay with `x=(W-w)/2+0`.
23. With `positionX: 100` in a `1080 × 1920` project exported at `2160 × 3840`, the emitted overlay
    reads `x=(W-w)/2+200`. Before the §6.2 fix it reads `+100`; that difference is the defect. This
    is case D of the script in item 24, not a one-time observation.
24. The three `EXPORT.md §1.8` transcripts diff **byte-for-byte** against the rebuilt graph. This is
    ...
    satisfying it.
25. A real Electron export of a vertical project at `1080 × 1920` produces a file `ffprobe` reports
    as `1080x1920`, with the source visible as a centred band and black above and below.
26. **The export dialog's Resolution select cannot render a value absent from its options.** In a
...
27. From a fresh store, `setProjectSize(1080, 1920)` then import 24 fps 1920 × 1080 footage, **on both
    paths, with different correct answers**:
    - **Electron** (`applyProbe`, a real probed rate) leaves
      `{ fps: 24, width: 1080, height: 1920, fpsLocked: true, sizeLocked: true, formatLocked: true }`.
    - **`dev:web`** (`importBrowserFile`, no rate measurable) leaves
      `{ fps: 30, width: 1080, height: 1920, fpsLocked: false, sizeLocked: true, formatLocked: false }`
      — the shape kept, the **rate still open** for the first import that can report one. Before
      §7.4's one-argument edit this reads `fpsLocked: true`, which is the defect: a rate nobody
      measured, locked forever.
28. **A rate adoption never shortens a clip silently.** In Electron from a fresh store: set Aspect to
    `Vertical 9:16`, import an audio file (nothing locks — `mediaSlice`'s adopt guard requires
    `kind === 'video'`), lay several minutes of it out at the default 30 fps, then import 24 fps
    video. Assert `readStore().fps === 24` and that if any clip's `durationFrames` decreased then
    `readStore().notice` is non-null with `title === 'Frame rate changed'`. Also assert the playhead
    is still within `selectTimelineDurationFrames + PLAYHEAD_TAIL_FRAMES` — the re-seek §7.3 adds.
29. `readStore().formatLocked === (readStore().fpsLocked && readStore().sizeLocked)` after every
    action in §7.3, in every order.

**Gates**

30. `npm run typecheck`, `npm run build`, `npm run check` all clean — and `npm run check` now runs
    four scripts, the fourth being `check-export-graph.mjs`.
31. `grep -rnE '#[0-9a-fA-F]{3,8}|rgba?\(|oklch\(|hsl\(' src/` returns zero hits outside
    `src/styles/tokens.css`.
32. `git status` shows changes only within §0's ownership list, and the four single-edit exceptions
    really are single edits: `git diff src/styles/tokens.css` is additions only and touches no
    existing declaration, `git diff src/state/mediaSlice.ts` changes one argument plus the comment
    above it, `git diff package.json` changes one string, and `scripts/check-export-graph.mjs` is a
    new file that imports nothing outside `dist-electron/` and `node:`.
...
- **No fallback matte, and no conditional limitation about bar colour.** §10.1 is part of this
  change (§5.4), so the preview's bars are black in all three themes. There is no "ships either
  way" state: `var(--frame-matte, …)` fails `npm run check` exactly as hard as the bare form.
